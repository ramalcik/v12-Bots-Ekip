...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    let ramalizm = new disbut.MessageButton().setStyle('red').setLabel('Kapat').setID('kapat')
...
\`\`\`
`), { buttons: [ramalizm] }).then(async (msg) => {
  ...
  client.on('clickButton', async (button) => {
    ...
    if(button.id === "kapat") {
      ...
      data.tagMode = false;
      data.save();
      msg.edit(embed.setDescription(`${green} taglı alım modu başarıyla deaktif edildi!`), {buttons: [ramalizm.setDisabled(true)]})
