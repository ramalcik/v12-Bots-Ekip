...
run: async (client, message, args, embed, prefix) => { 
  ...
  let ayar = await serverSettings.findOne({
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}kadın <@ramal/ID> <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}kadın <@user/ID> <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
