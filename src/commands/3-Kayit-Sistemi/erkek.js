...
run: async (client, message, args, embed, prefix) => { 
  ...
  let ayar = await serverSettings.findOne({
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}erkek <@user/ID> <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}erkek <@ramal/ID> <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
    ...
    ${uye.toString()} üyesinin ismi başarıyla \`${(setName)}\` olarak Kayıt Edildi.`)
