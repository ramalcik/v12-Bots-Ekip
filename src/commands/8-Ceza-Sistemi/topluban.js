...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    collector.on("collect", async () => {
        await ramalcik.edit(`${green} ${members.length} adet kullanıcı başarıyla yasaklandı.`);
    ...
    collector.on("end", (_, reason) => {
        ...
        if (reason === "time")
            ramalcik.edit("10 saniye geçtiği için işlem iptal edildi.");
    });
