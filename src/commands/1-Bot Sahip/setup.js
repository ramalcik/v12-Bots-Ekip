...
///"style":4 kırmızı
    ...
    let msg = await message.channel.send({ components: [row1, row2], embed: ramalmsg})
    ...
    collector.on("collect", async (button) => {
      if(button.id == 'setup1'){
        ...
        .setDescription(`
        \`>\` ${prefix}setup man @erkekrol1 @erkekrol2
        \`>\` ${prefix}setup woman @kadınrol1 kadınrol2
        \`>\` ${prefix}setup unregister @kayıtsızrol
        \`>\` ${prefix}setup vip @viprol
        \`>\` ${prefix}setup booster @boosterRolu
