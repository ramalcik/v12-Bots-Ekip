...
module.exports = async (member) => {
  ...
  if (!inviteMemberData) {
    channel.wsend(`${client.emojis.cache.find(x => x.name === "leave")} \`${member.user.tag}\` sunucumuzdan ayrıldı ama kim tarafından davet edildiğini bulamadım.`);
  } else {
    ...
    channel.wsend(`${client.emojis.cache.find(x => x.name === "leave")} \`${member.user.tag}\` sunucumuzdan ayrıldı. \`${inviter.tag}\` tarafından davet edilmişti.`);
